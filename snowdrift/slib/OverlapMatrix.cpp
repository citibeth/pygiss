...
bool OverlapMatrix::add_pair(GridCell const *gc0, GridCell const *gc1)
{
	...
	overlap->set(gc0->index - grid1->index_base,
		gc1->index - grid2->index_base, area);
...
void OverlapMatrix::set_grids(Grid *_grid1, Grid *_grid2)
{
	...
	overlap.reset(new VectorSparseMatrix(SparseDescr(n1, n2, 1)));
	int i=0;
	...
	for (auto ii1=grid1->cells().begin(); ii1 != grid1->cells().end(); ++ii1) {
		...
		++i;
		if (i % 1000 == 0) {
			printf("Processed %d of %d from grid1, total overlaps = %d\n", i+1, grid1->cells().size(), overlap->size());
		}

	}

	// Sort the matrix
	overlap->sort(SparseMatrix::SortOrder::ROW_MAJOR);
}
...
void OverlapMatrix::to_netcdf(std::string const &fname)
{
	...
	auto ncd = overlap->netcdf_define(nc, "overlap");
